...
public class ApiExceptionHandler {
    ...
    ProblemDetail duplicate(DuplicateIsbnException ex) { return ProblemDetail.forStatusAndDetail(HttpStatus.CONFLICT, ex.getMessage()); }
    ...
    ProblemDetail invalid(MethodArgumentNotValidException ex) {
        ...
        detail.setProperty("errors", ex.getBindingResult().getFieldErrors().stream().map(e -> e.getField() + ": " + e.getDefaultMessage()).toList());
        return detail;
    }
}
