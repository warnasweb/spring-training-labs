...
def request(path,body=None,key=None,method=None):
    headers={"Content-Type":"application/json","X-Correlation-ID":"day8-security-smoke"}
    if TOKEN:
...
for port in range(8180,8186):
    ...
    while True:
        ...
        time.sleep(1)
TOKEN=request("/auth/token", {"username":"trainer", "password":"training"})["accessToken"]
