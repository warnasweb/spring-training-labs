...
public class InventoryClient {
 ...
 public InventoryClient(RestClient.Builder builder,@Value("${inventory.url:http://localhost:8082}") String url) {
  var factory=new JdkClientHttpRequestFactory(HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(1)).build());
  factory.setReadTimeout(Duration.ofSeconds(2));http=builder.baseUrl(url).requestFactory(factory).build();
 }
 ...
 public boolean reserve(UUID order,UUID product,int quantity,String correlation) {
  ...
  var r=http.post().uri("/inventory/reservations").header("X-Correlation-ID",correlation)
 ...
 public void release(UUID order,String correlation) { http.delete().uri("/inventory/reservations/{id}",order).header("X-Correlation-ID",correlation).retrieve().toBodilessEntity(); }
